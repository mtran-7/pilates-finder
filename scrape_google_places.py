...
# Main function to scrape Pilates studios for all cities listed in cities.json

def scrape_pilates():
    """Scrape Pilates studios for all cities listed in cities.json."""
    # Load cities from cities.json
    with open("cities.json", "r") as file:
        states_and_cities = json.load(file)
    ...
    for state, cities in states_and_cities.items():
        for city in cities:
            ...
            city_data = [result for result in results if result]
            all_data.extend(city_data)

    # Save all results to a JSON file
    with open("pilates_studios.json", "w") as file:
